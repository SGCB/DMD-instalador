...
NOTE : Before launching the installer step 3, a necessary analysis of the collection must be performed, its
characteristics and description policies that the organization has adopted. The tasks of creating authorities
which takes ASKOSI are generic, since it started from the premise that each repository needs are different.
These tasks can be realized in the creation of a proper form for the introduction of new authorities ( input_forms.xml )
modification of the method of submission, simplifying if necessary ( item_submission.xml ) or decision
to hide authorities collections to unregistered users...
The Plugin ASKOSI serves as support for the creation and control of controlled vocabularies carried out in section 3 of this
installer. If you want to unify collections of authorities should be done in this step.
However, ASKOSI configuration files can be modified to suit the repository particularities, since they do
not overwrite the DSpace ones, that are stored in a installer directory( prior review and modification by the administrator) before the production stage.
